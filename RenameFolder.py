...
root_folder_path = r'\\ZIMA\ZimaShare\Afbeeldingen\Fotos All Years'
root_folder = os.listdir(root_folder_path)
...
for folder_name in root_folder:
    ...
    current_subfolder_name = os.path.join(root_folder_path, folder_name)

    if os.path.isfile(current_subfolder_name):
        continue

    files = os.listdir(current_subfolder_name)
    ...
    for file_name in files:
        ...
        old_file_path = os.path.join(current_subfolder_name, file_name)
        ...
        if metadata is None:
            print(f"EXIF metadata not found in file: {folder_name}")
            failedAttempts -= 1
            if failedAttempts != 0:
                continue
        ...
        if metadata is not None:
            ...
            if 36867 in metadata.keys():
                ...
                print(f"Date not found in file: {old_file_path}")     
                failedAttempts -= 1
                if failedAttempts != 0:
                    continue
        ...
        if failedAttempts == 0:
            dateobj= os.path.getmtime(old_file_path)
            date_taken =  datetime.fromtimestamp(dateobj)
            date_taken = date_taken.__format__("%Y:%m:%d %H:%M:%S")
            #date_taken = os.path.getctime(old_file_path)
        ...
        year_taken = date_taken.strftime("%Y")
    
        # we have what we need
        break    # break here

    if len(year_taken ) > 0:
        if folder_name.startswith(year_taken):
            continue

        new_folder_name = year_taken + " " + folder_name
        new_folder_path = os.path.join(root_folder_path, new_folder_name)

        if os.path.exists(new_folder_path):
            os.rename(new_folder_path, new_folder_path + ' - DUPLICATE NAME')
            print(f"found already excisting name: {new_folder_name} added : DUPLICATE NAME to new foldername")
        else:
            os.rename(current_subfolder_name, new_folder_path)
            print(f"renamed: {folder_name} to {new_folder_name} in {root_folder_path}")
